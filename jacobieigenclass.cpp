...
//using namespace std;   
JacobiEigensClass::JacobiEigensClass(std::vector<double> inputMatrix, double epsilon) {
    this->epsilon = epsilon;
    N = sqrt(inputMatrix.size());
    maxIterations = N*N*N;
...
void JacobiEigensClass::rotateVarVector() {
    double c, s;
    int i_max=(int)(currentIndexOfMaxElement / N);
    int j_max=currentIndexOfMaxElement - (int)(currentIndexOfMaxElement / N) * N;
